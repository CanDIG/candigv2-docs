...
The file should contain an array of dictionaries, where each item represents a single file. Each dictionary specifies important information about the genomic file and how it links to the ingested clinical data. The structure of this dictionary and options for enumerated fields are specified in the SequencingIngest [openapi schema](https://github.com/CanDIG/candigv2-ingest/blob/stable/ingest_openapi.yaml), an [example file](https://github.com/CanDIG/candigv2-ingest/blob/stable/tests/genomic_ingest.json) exists within the test files and a commented example in the expandable section below:
    ...
    "analyses": [
        {   ## Example linking to genomic and index files in s3 storage to a single sample
            ...
            "main": {                         # location and name of the main genomic file, bam/cram/vcf
                "access_method": "https://endpoint_url/s3_bucket_name/release/20130502/ALL.chr22.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz?public=true",
...
| ----------------------------------------------------------------------------------------------------- | ----------------------- | ------------------- | ------------------------ |
| [FastQ Format](https://maq.sourceforge.net/fastq.shtml)                                               | `.fastq` `.fq.gz` `.fq` | n/a                 | n/a                      |
...
| ----------------------------------------------- | --------------------------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Matrix with genes as rows and counts as columns | doesn't matter as long as it is a tab-delimited text file | `sequence_annotation`    | `subtype`==`expression_count`,`gene_id_column`, `length_column`, `count_column`, `norm_column`, `norm_method` (see below for detailed descriptions) |
    ...
    "analyses": [
        {
            ...
            "metadata": {
                ...
                "analysis_attribute": {
                    ...
                    "norm_column": "<name of column containing normalized values in tsv>",
...
:::tip
All options for enumerated fields can be found in the [ingest schema](https://github.com/CanDIG/candigv2-ingest/blob/stable/ingest_openapi.yaml)
:::
