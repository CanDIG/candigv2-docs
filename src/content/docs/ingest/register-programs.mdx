...
Before any program can be ingested into CanDIG, it needs to be registered. To first register a program, you must be either a site admin or a site curator. During program registration, you can also assign program curators and team members to the program. For more information about user roles in CanDIG, see [User roles in CanDIG](/candigv2-docs/user-roles/roles-overview/). If you need to be made a site curator or program curator, please contact your local site administrator to assign your user profile the appropriate role.
...
## How to register many Programs

If you want to register many programs, we have a [helper script](https://github.com/CanDIG/candigv2-ingest/blob/develop/helper_scripts/submit_program_registrations.py) where you can specify a list of programs, as well as the `team_members` and `program_curators` that should be added to the registration.
