...
In the course of reingesting data, it is possible for sequencing file objects to become out of sync with clinical data and which program they are assigned to. For example, if a program is split between releaseable and non-releaseable data. While reingesting clinical data is fairly fast, if a submitter doesn't want to resubmit and reindex genomic files, they can use some helper scripts to re-assign the program for specific objects.
