...
<Aside type="tip">
  ...
  and try turning the indexer off and on again with ```bash curl --request POST
  \ --url "$CANDIG_URL/genomics/htsget/v1/indexer?status=OFF" \ --header
  "Authorization: Bearer "$TOKEN ``` ```bash curl --request POST \ --url
  "$CANDIG_URL/genomics/htsget/v1/indexer?status=ON" \ --header "Authorization:
  Bearer "$TOKEN ``` If indexing does not start, follow the steps in 2. to check
  your indexing queue
</Aside>
