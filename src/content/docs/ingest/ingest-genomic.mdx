...
- **Runs** describe the raw sequencing runs that were performed and usually link to fastq files
  ...
  "analyses": [
    {
      ...
      "main": {
        "name": "string",
        "access_method": "https://endpoint_url/s3_bucket_name/HG00096.vcf.gz"
      },
      "index": {
        "name": "string",
        "access_method": "https://endpoint_url/s3_bucket_name/HG00096.tbi"
      },
      "samples": [
        {
          "experiment_id": "SEQ_0001",
...
:::tip
There are a couple of optional parameters that you can specify to this endpoint:

- `overwrite`: `true`/`false`, default is false. If you submit a file with genomic linkages that already exist it won't overwrite existing entries unless you specify `true` here. If a file contains both existing and new linkages, it will
- `batch_size`: integer - specify number of objects to ingest at a time, usually not needed
...
### 1. Look at the indexer status

The status of the indexer will give you a list of failed and pending files that are waiting to be indexed. The API call to view the indexer's status is:

```bash
curl --request GET \
  --url $CANDIG_URL'/genomics/htsget/v1/indexer' \
...
:::tip
If indexing is not proceeding as you expect, you can check your indexer status
and try turning the indexer off and on again with
...
```bash
curl --request GET \
  --url $CANDIG_URL'/drs/ga4gh/drs/v1/programs/local-SYNTH_01/status' \
  --header 'Authorization: Bearer '$TOKEN
{
	"index_complete": [
