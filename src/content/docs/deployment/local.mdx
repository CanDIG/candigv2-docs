...
CanDIG can be installed and deployed as below for development situations where no real data will ever be ingested into the system. For critical differences in production deployments, please see the <a href="/candigv2-docs/deployment/production/">Guide to CanDIG production deployments</a>.
...
:::danger
If you are deploying a production level CanDIG deployment that will host real clinical data, see the [production deployment guide](/candigv2-docs/deployment/production) for all the settings that need to be updated to ensure the deployed platform is secure.
:::
...
Next up, you might like to take a look at the documentation for [ingesting data](/candigv2-docs/ingest) as well as [Interacting with the stack using Make](/candigv2-docs/deployment/interact-with-the-stack). If you are looking to set up a Production node, please read through the [production deployment guide](/candigv2-docs/deployment/production).
...
Edit your firewall settings to allow connections from those addresses:
