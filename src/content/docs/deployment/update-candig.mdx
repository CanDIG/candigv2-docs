...
1.  Backup your existing deployment using the steps on the page: [Backing up and restoring CanDIG data](/candigv2-docs/deployment/backup-restore-candig/). This should be done regardless of whether you think the deployment will destroy any data, just in case anything goes wrong. Things you will want to back up are:

    - ingested genomic and clinical data - Guide here: [Back up postgres databases](/candigv2-docs/deployment/backup-restore-candig/#backing-up-postgres-databases)
    - log files - Guide here: [backup logfiles](/candigv2-docs/deployment/backup-restore-candig/#backing-up-logs)
    - user authorizations - Guide here: [backup secrets and authorizations](/candigv2-docs/deployment/backup-restore-candig/#backing-up-secrets-and-authorization-data)
    ...
    1.  [Get a site admin token](/candigv2-docs/user-roles/assign-roles/#getting-an-api-token) and in a terminal session, store in a variable called `TOKEN`, store your candig domain in a variable called `CANDIG_URL`
...
9.  Run integration tests to ensure all the services are running as expected
    ...
    1.  To reinstate `site_admin@test.ca` as a site admin user, from the CanDIGv2 repo in a terminal, first get a token:
    ...
    If tests fail, check out the [troubleshooting section](/candigv2-docs/deployment/stack-troubleshooting/) or [reach out for help](https://github.com/CanDIG/CanDIGv2/issues/new/choose).
