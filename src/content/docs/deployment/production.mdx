...
| `CANDIG_USER_KEY=preferred_username`                                                                         | This may need to be changed to `email` if connecting to an LDAP, it is important that this value coincides with the email address of users as this is how permissions are granted to other users through DAC and program authorizations |
...
| `TYK_ANALYTICS_FROM_EMAIL=YOUR-ADMIN-EMAIL@email.ca`                                                         | Update to a relevant email address                                                                                                                                                                                                      |
...
:::tip
See [User roles](/candigv2-docs/user-roles/roles-overview/) for a full description of the available user roles in CanDIG.
:::

### Adding a site curator

See [User Roles](/candigv2-docs/user-roles/assign-roles/#assign-the-site-curator-role-)
...
:::caution
It is important that the `CANDIG_USER_KEY` specified in your `.env` file coincides with an email address in the `jwt` that is issued from your LDAP
:::

:::tip
CanDIG was developed with the assumption that deployers would connect to an external LDAP for authentication so there is limited support for deployers that choose to manually create users directly in Keycloak. Please ensure you are well-versed in Keycloak and how to back up and restore as CanDIG does not provide any tools or support for this.
:::

## Configurations for Genomic Ingest

To allow genomic ingest there are some extra configuration steps required. Please see [Configure connection to Genomic Data source/s](https://candig.github.io/candigv2-docs/ingest/prepare-genomic/#a-configure-connection-to-genomic-data-sources) for information on how to configure for NFS or s3 type file linkages.
...
Access to patient level data through specific program or DAC authorization is managed by the node that hosts the data for that program. For example, if a user from UHN needs to be given authorization to a program hosted within the BC node, a site administrator from BC will need to submit a [DAC authorization](https://candig.github.io/candigv2-docs/user-roles/dac-authorization/) for each program the user needs access to with the start and end dates their access should be active for.

## Backing up production data

It is not expected that a CanDIG instance would hold the only copy of any ingested data. However, recognising that the ETL and ingest process takes significant time and effort, it is a good idea to regularly backup all data stored in CanDIG. Steps for how to do this can be found in [Backing up and restoring CanDIG data](/candigv2-docs/deployment/backup-restore-candig/)
